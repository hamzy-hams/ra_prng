#!/usr/bin/env python3
"""verify_parity.py -- bit-for-bit parity check between scrambler_addressable.c
(compiled binary) and scrambler_addressable.py, across several keys and input
sizes. Mirrors the addressable-init research's Tahap 2 discipline: a C
implementation is not trusted until it matches a faithful Python port
byte-for-byte.

Usage: python3 verify_parity.py
Requires ./scrambler_addressable (compiled) to exist in this directory.
"""
...
BINARY = HERE / "scrambler_addressable"
...
def run_c(tokens: list[int], key: int) -> list[int]:
    in_path = HERE / "_parity_in.txt"
    out_path = HERE / "_parity_out.txt"
...
def main():
    if not BINARY.exists():
        print(f"Error: {BINARY} tidak ditemukan. Build dulu dengan:\n"
              f"  gcc -std=c11 -O3 -o scrambler_addressable scrambler_addressable.c",
              file=sys.stderr)
        sys.exit(1)
    ...
    for path in (HERE / "_parity_in.txt", HERE / "_parity_out.txt"):
        if path.exists():
            path.unlink()
    ...
    if failures:
        sys.exit(1)
    print("PASS: scrambler_addressable.c bit-identical to scrambler_addressable.py "
