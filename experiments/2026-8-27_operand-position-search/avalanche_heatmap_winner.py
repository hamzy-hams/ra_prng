"""Avalanche-effect heatmap for the winning operand-position rewiring
found by operand_search.py: Wiring(a_xor_operand="d", c_shift_operand="b",
rotc_amount_source="b", rotc_xor_operand="a") -- i.e. baseline.c
(pruned_winner) with exactly one XOR operand changed:
`a = (b ^ o) ^ (cons + a)` -> `a = (d ^ o) ^ (cons + a)`.

Direct port of
../2026-8-26_operation-pruning-research/avalanche_heatmap_pruned_winner.py,
retargeted at this wiring instead of baseline.c's fixed formula, so the two
are visually and numerically comparable (same axes, same cross-check
structure). See RESULTS.md for the full per-bit table this produces.
"""
...
MASK = 0xFFFFFFFF
BASE_SEED = 1
WINNER = Wiring(a_xor_operand="d", c_shift_operand="b",
...
def winner_reseed(M: list[int], L: list[int]) -> int:
    """Mirrors winner_wired.c's ra_reseed()/ra_hash() exactly (identical to
    baseline.c's -- rewiring only touches ra_permutation_cycle's a/b/c
    chain, never reseed)."""
    for i in range(256):
        M[i] ^= L[i]
    new_cons = 0
...
def winner_capture(seed: int, cycle: int = 1) -> list[int]:
    ...
    hand-rolled mirror of winner_wired.c's ra_core loop (used purely as an
    independent cross-check against wired_prng.py's stream())."""
    if cycle < 1:
...
def plot_single(matrix, stats, out_png: Path) -> None:
    ...
    plt.title(
        "Avalanche Effect Heatmap: winner_wired (a_xor=d, c_shift=b, "
...
def plot_comparison(matrix1, stats1, matrixN, statsN, cycle: int, out_png: Path) -> None:
    ...
    fig.suptitle(
        "Avalanche Effect: winner_wired (a_xor=d, c_shift=b, rotc_amount=b, "
...
def main() -> None:
    ...
    if args.cycle == 1:
        ...
        out_png = Path(__file__).parent / "avalanche_heatmap_winner.png"
        ...
        out_png = Path(__file__).parent / f"avalanche_heatmap_winner_cycle1_vs_cycle{args.cycle}.png"
