"""Avalanche-effect heatmap for the TRUE original algorithm (ALL_OPS,
HASH_ACCESS_STRIDED + HASH_SELFIDX) -- src/ra_prng2/c/ra_prng2.c's exact
`ra_permutation_cycle`/`ra_reseed`.

Port of experiments/2026-8-26_operation-pruning-research/
avalanche_heatmap_pruned_winner.py onto this experiment's actual subject:
the unpruned, unrewired original. NOT `baseline.c` in this folder --
despite the name, `baseline.c` is a copy of the *pruning* experiment's
winning candidate (`TAP6,TAP7,ROT_C,SHR13`), not the original algorithm
(see HANDOVER.md, "Baseline to preserve / use as reference"). The
genuine original lives at src/ra_prng2/c/ra_prng2.c /
src/ra_prng2/python/ra_prng2.py and is never modified here.

Why this script exists: the pruned-winner heatmap (sibling folder)
revealed that seed bits 5 and 6 are nearly avalanche-dead (253/255
output positions show 0 bit difference when either bit is flipped, vs.
~15-16/32 for other bits) -- a defect invisible to quality_gate.py's
avalanche_gate(), which only reports one number averaged across all 32
bits (0.418, still inside the [0.3, 0.7] pass band). This script exists
to measure -- not assume -- whether the *original*, unpruned algorithm
has the same weakness or is genuinely more even across both axes (bit
flip x output position), which is the open question motivating this
experiment's whole premise (rewiring operand positions to find a
better-diffusing starting point for a future pruning round).

original_capture() implements the full 5-line update chain (all 8 taps,
ROT_A/ROT_B/ROT_O/ROT_C, SHL9/SHR18/SHL14, MULT_REDUCE):
    o = 0; for e in 0..7: o ^= (M[(i+e)&0xFF] << e)
    a = rot32(b ^ o, d) ^ (cons + a)
    b = rot32(cons + a, i) ^ (o + d)
    o = (rot32(a ^ o, i) << 9) ^ (b >> 18)
    c = rot32(((o + c) << 14) ^ (b >> 13) ^ a, b)
    d = (c * (i+1)) >> 32
Reseed hashes with HASH_SELFIDX + HASH_ACCESS_STRIDED (depth 32):
    out[i] = N[N[i] & 0xFF] ^ XOR_{j=0..31} N[j*8 + i]

Being a much longer formula than the 4-op pruned-winner version (higher
risk of a transcription bug), every run cross-checks itself twice before
trusting any result:
  ...
  (b) against src/ra_prng2/python/ra_prng2.py's paper-exact ra_core() --
      ...
      cross-check pruned_prng.py._self_check() already performs.
...
src/ra_prng2/python the same way).
"""
...
def original_capture(seed: int, cycle: int = 1) -> list[int]:
    """Cycle `cycle`'s (1-indexed) 255-step permutation-cycle output for
    the full original algorithm. cycle=1 is the first cycle, no prior
    reseed; cycle=N runs N-1 reseeds first."""
    if cycle < 1:
...
def analyze_cycle(cycle: int):
    ...
    hand_matrix, hand_fraction = compute_avalanche_matrix(
        lambda seed: original_capture(seed, cycle)
    )
...
def plot_single(matrix, fraction: float, out_png: Path) -> None:
    ...
    plt.title("Avalanche Effect Heatmap: original algorithm (ALL_OPS)")
...
def main() -> None:
    ...
    if args.cycle == 1:
        ...
        out_png = _HERE / "avalanche_heatmap_original.png"
        ...
        out_png = _HERE / f"avalanche_heatmap_original_cycle1_vs_cycle{args.cycle}.png"
