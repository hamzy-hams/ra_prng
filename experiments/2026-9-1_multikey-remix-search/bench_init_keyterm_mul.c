...
static void ra_init_state_keyterm_mul(uint32_t *L, uint32_t *M, uint32_t key){
    uint32_t keyterm_l = fmix32(0x9e3779b7u * key);
    uint32_t keyterm_m = fmix32(0x06a0dd9bu * key);
    for(int i=0;i<256;++i){
        uint32_t r=key^(uint32_t)i;
        uint32_t l=(uint32_t)(i*0x9e3779b7u)*keyterm_l;
        uint32_t m=(uint32_t)(i*0x06a0dd9bu)*keyterm_m;
        L[i]=rot32(l,r); M[i]=rot32(m,r);
    }
}
...
static uint32_t core_keyterm_mul(uint32_t key, size_t rng, uint32_t *sink){
    alignas(64) uint32_t L[256],M[256]; ra_init_state_keyterm_mul(L,M,key);
    uint64_t count=rng; ra_cycle(key,0,M,L,&count,sink); return key;
}
int main(int argc, char **argv){
    ...
    clock_gettime(CLOCK_MONOTONIC,&t0);
    for(size_t k=0;k<nkeys;++k){uint32_t key=(uint32_t)(k*0x9E3779B9u+12345u); core_keyterm_mul(key,K,&sink);}
    clock_gettime(CLOCK_MONOTONIC,&t1);
    double t_km=(t1.tv_sec-t0.tv_sec)+(t1.tv_nsec-t0.tv_nsec)/1e9;
    double words=(double)(nkeys*K);
    printf("K=%zu  baseline=%.4f ns/word  keyterm_mul=%.4f ns/word  overhead=%.2fx  sink=%u\n",
           K, t_base/words*1e9, t_km/words*1e9, t_km/t_base, sink);
    return 0;
}
