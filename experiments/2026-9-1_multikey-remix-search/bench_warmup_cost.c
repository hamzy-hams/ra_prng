// bench_warmup_cost.c -- DIAGNOSTIC ONLY. Measures fixed per-reinit cost of
// adding a discarded warm-up cycle + reseed, at various block sizes K
// (words produced per key before switching to the next key), matching
// bench_ra_core.c's reinit-sweep methodology. Not linked to ra_core.c.
#include <stdio.h>
#include <stdlib.h>
#include <stdint.h>
#include <string.h>
...
static void ra_cycle(uint32_t cons, size_t it, const uint32_t *M, uint32_t *L,
                      uint64_t *count, uint32_t *sink) {
...
static uint32_t core_baseline(uint32_t key, size_t rng, uint32_t *sink) {
    ...
    uint64_t count=rng;
    ra_cycle(key,0,M,L,&count,sink);
    return key;
}
static uint32_t core_warmup(uint32_t key, size_t rng, uint32_t *sink) {
    alignas(64) uint32_t L[256],M[256];
    ra_init_state_orbit(L,M,key);
    uint64_t wc=255;
    uint32_t discard=0;
    ra_cycle(key,0,M,L,&wc,&discard);
    uint32_t cons=ra_reseed(M,L);
    uint64_t count=rng;
    ra_cycle(cons,1,M,L,&count,sink);
    (void)discard;
    return cons;
}
...
int main(int argc, char **argv) {
    ...
    size_t total_words = 20000000; // fixed total work per measurement
    ...
    for (size_t k=0;k<nkeys;++k) {
        uint32_t key = (uint32_t)(k*0x9E3779B9u + 12345u);
        core_baseline(key, K, &sink);
    }
    ...
    for (size_t k=0;k<nkeys;++k) {
        uint32_t key = (uint32_t)(k*0x9E3779B9u + 12345u);
        core_warmup(key, K, &sink);
    }
    clock_gettime(CLOCK_MONOTONIC,&t1);
    double t_warm = (t1.tv_sec-t0.tv_sec)+(t1.tv_nsec-t0.tv_nsec)/1e9;

    double words = (double)(nkeys*K);
    printf("K=%zu  baseline=%.4f ns/word  warmup=%.4f ns/word  overhead=%.2fx  sink=%u\n",
           K, t_base/words*1e9, t_warm/words*1e9, t_warm/t_base, sink);
    return 0;
}
