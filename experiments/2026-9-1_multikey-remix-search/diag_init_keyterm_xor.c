// diag_init_keyterm_xor.c -- DIAGNOSTIC ONLY. Same per-key hashed term as
// diag_init_keyterm_avalanche.c's additive candidate, but combined with the
// per-index step by XOR instead of `+`: hash ONLY the key-dependent term
// (`0x9e3779b7u*key` for L, `0x06a0dd9bu*key` for M) ONCE per reinit --
// since key and the constants never change within one reinit, this term is
// computed once and reused across all 256 elements. Rotation (`r = key^i`,
// still the RAW key, not hashed) and the `i*C` term stay exactly as in the
// current production formula.
//
// Risk flagged before testing (not assumed): structurally close to the
// additive variant that already failed -- XOR-ing in a per-key constant
// doesn't touch the repeating relationship between adjacent M[i]/M[i+1]
// either, which first-cycle analysis pointed to as the real defect driver.
...
static void ra_init_state_keyterm_xor(uint32_t *L, uint32_t *M, uint32_t key) {
...
static void ra_core_keyterm_xor(uint32_t key, size_t block_len, FILE *raw_stream) {
    if (block_len == 0) return;
    alignas(64) uint32_t L[256], M[256];
    ra_init_state_keyterm_xor(L, M, key);
...
int main(int argc, char **argv) {
    ...
    while (remaining > 0) {
        ...
        ra_core_keyterm_xor(key, block_len, stdout);
        remaining -= block_len;
        ++block_idx;
    }
    return 0;
