...
## Corpus Check
- 2 files · ~515,259 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1309 nodes · 2038 edges · 132 communities (106 shown, 26 thin omitted)
- Extraction: 96% EXTRACTED · 4% INFERRED · 0% AMBIGUOUS · INFERRED: 83 edges (avg confidence: 0.89)
- Token cost: 63,885 input · 0 output
...
- Toy Sweep rows=2 Config Log
- Standalone File Node
- Toy-Scale Op/Wiring Port Decision
- Short Transient Tail Finding
- ra-prng Package Metadata
- ra_prng2 Changelog
- ra_prng3 Changelog
- ra_prng3 Changelog
- Image Encrypting Sub-project
- Image Encrypting Sub-project
- Periodicity Rerun stderr Log
- Periodicity Rerun stdout Log
- Short Transient Tail Finding

## God Nodes (most connected - your core abstractions)
1. `ra_prng2 (Array-Based PRNG)` - 29 edges
2. `HANDOVER.md (Addressable State Init Research Spec)` - 25 edges
3. `State-Update Mechanism Research Results (Phase 1-3)` - 23 edges
4. `Params` - 22 edges
5. `Parallelization Research Handover` - 21 edges
6. `Candidate` - 20 edges
7. `Operand Position Search Experiment Results` - 19 edges
8. `Wiring` - 18 edges
9. `stream()` - 17 edges
10. `pruned_winner 4-Operation Fast Variant` - 14 edges
...
- **5-Stage Addressable-Init Research Flow (Tahap 0-5)** — experiments_2026_8_30_addressable_init_research_handover_tahap_0_mechanism_test, experiments_2026_8_30_addressable_init_research_handover_tahap_1_freeze_spec, experiments_2026_8_30_addressable_init_research_handover_tahap_2_implementation, experiments_2026_8_30_addressable_init_research_handover_tahap_3_statistical_validation, experiments_2026_8_30_addressable_init_research_handover_tahap_4_speed_benchmark, experiments_2026_8_30_addressable_init_research_handover_tahap_5_optimization [EXTRACTED 1.00]
- **Tahap 0 Validates Core Hypothesis via Candidate Formulas** — experiments_2026_8_30_addressable_init_research_handover_tahap_0_mechanism_test, experiments_2026_8_30_addressable_init_research_handover_addressable_init_hypothesis, experiments_2026_8_30_addressable_init_research_handover_kandidat_1_l_only_change, experiments_2026_8_30_addressable_init_research_handover_kandidat_2_independent_lock, experiments_2026_8_30_addressable_init_research_handover_kandidat_3_negative_control_rotation, experiments_2026_8_30_addressable_init_research_handover_kandidat_4_seed_counter_mult [INFERRED 0.85]
...
Cohesion: 0.08
Nodes (39): load_means(), main(), avalanche_gate_min_bit metric (per-bit floor 0.2, band [0.3,0.7]), check_no_repeat(), main(), Path, Full-scale (real 256x32-bit) practical periodicity sanity check, per…, build() (+31 more)
...
Cohesion: 0.10
Nodes (39): Four-Axis Validation: periodicity, PractRand, avalanche, speed, permute + xor_fold State-Update Mechanism (kept unchanged), FILE, main(), ra_core(), ra_hash(), ra_init_state(), ra_permutation_cycle() (+31 more)
...
Cohesion: 0.13
Nodes (22): Dieharder Test Results (ra_prng2 original), RNGing perf stat (ra_prng2 original), Shuffling perf stat (ra_prng2 original), TestU01 BigCrush Results (ra_prng2 original), Dieharder Test Results (ra_prng3), ent Entropy Test Results (ra_prng3), NIST STS Test Results (ra_prng3), RNGing perf stat (ra_prng3) (+14 more)
...
Cohesion: 0.11
Nodes (18): BigCrush (TestU01), Bit Shifting, Rationale: Branchless Design for CPU Pipeline Efficiency, ChaCha20, Daniel Lemire, ent Entropy Tool, John Walker, Rationale: Small 2KB State Fits L1 Cache for High IPC (+10 more)
...
Cohesion: 0.20
Nodes (10): other_winners_practrand.sh script, Avalanche seed diffusion (1-bit cons diff spreads through L/M in 255 steps), main(), Q1 Method B: interleave K independent `--stream <seed> <n>` streams word-by-…, run_interleave_practrand(), Method A: direct cross-correlation (Pearson + Bonferroni), Method B: interleaved PractRand, PractRand over-supply pattern (avoid short-by-N-words failure via BrokenPipeError cutoff) (+2 more)
...
Cohesion: 0.24
Nodes (11): Bonferroni correction (family-wise alpha 0.01 for K x K pair testing), Bounded capture: run `binary --stream seed n`, return n uint32 values. Suitable…, stream_values(), analyze_group(), build_group(), main(), ndarray, Q1 Method A: Pearson cross-correlation between pairs of independent `--stream… (+3 more)
...
Cohesion: 0.39
Nodes (7): build(), gen_c(), main(), Generate + compile the phase-2 (M-only) state-update-mechanism spectrum, per…, The M mechanism only affects the reseed fold (once per 255 steps) -- the first…, verify_first_cycle_matches_control(), Path
...
Cohesion: 0.29
Nodes (6): ensure_ra_prng2_cli(), Shared helpers for the parallelization-research harness. Never modifies…, Streaming capture: caller reads/closes proc.stdout incrementally. Use for large…, Compile the optional paper-exact comparator into this folder, if not already…, stream_popen(), Popen
...
Cohesion: 0.33
Nodes (6): Hamas A. Rahman, ACM (Association for Computing Machinery), CSAI 2025 (9th Int'l Conf. on Computer Science and AI), EI Compendex, ra_prng2 and Beyond (Paper CA-162), Scopus
...
### Community 100 - "xoshiro256** RNG Speed Benchmark Result"
Cohesion: 1.00
...
### Community 101 - "cpython/__init__.py"
Cohesion: 0.67
Nodes (3): Rationale: Golden-Ratio Constants Reduce Collisions, Dead States, Short Cycles, PCG32, Seed and Array Initialization (Algorithm 1)
...
## Knowledge Gaps
- **153 isolated node(s):** `other_winners_perf.sh script`, `perf_state_update.sh script`, `perf_state_update_m.sh script`, `toy_probe_phase3.sh script`, `toy_sweep_m.sh script` (+148 more)
...
- **What is the exact relationship between `Hamas A. Rahman` and `hamzy hams (GitHub maintainer)`?**
  _Edge tagged AMBIGUOUS (relation: conceptually_related_to) - confidence is low._
- **Why does `Avalanche Effect Heatmap (winner_wired_v2)` connect `scc_test.py` to `Wiring`, `Operand Position Search Experiment Results`?**
  _High betweenness centrality (0.084) - this node is a cross-community bridge._
- **Why does `Avalanche Effect (Bit-Flip Sensitivity)` connect `scc_test.py` to `Design decisions (read before touching `pruned_wired_toy_prng.py`)`?**
  _High betweenness centrality (0.073) - this node is a cross-community bridge._
- **Why does `ra_prng2 (Array-Based PRNG)` connect `Design decisions (read before touching `pruned_wired_toy_prng.py`)` to `pcg32.c`, `cpython/__init__.py`, `ra_prng3 PRNG Generator`, `State Transition Operator F`, `Cycle 3 Avalanche Fraction (0.470588)`, `ra_prng2 Package Changelog`, `ra-prng`, `scc_test.py`, `Avalanche Effect Heatmap: pruned_winner (TAP6, TAP7, ROT_C, SHR13)`?**
...
- **What connects `other_winners_perf.sh script`, `perf_state_update.sh script`, `perf_state_update_m.sh script` to the rest of the system?**
